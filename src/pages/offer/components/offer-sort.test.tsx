...
import { OffersSort } from './offer-sort';
...
describe('OffersSort Component', () => {
  ...
  it('should highlight active sort option', () => {
    ...
    const activeOption = screen.getAllByText('Popular')[1];
    expect(activeOption.closest('.places__option')).toHaveClass('option-active-class');
  });

  it('should change active option when different sort is selected', async () => {
    const user = userEvent.setup();
    ...
    const newStore = createMockStore('Price: low to high');
    rerender(
    ...
    const newActiveOption = screen.getAllByText('Price: low to high')[1];
    expect(newActiveOption.closest('.places__option')).toHaveClass('option-active-class');
  });
  ...
  it('should apply correct CSS classes from module', () => {
    const { container } = render(
      <Provider store={store}>
        <OffersSort />
      </Provider>
    ...
    const caption = screen.getByText('Sort by');
